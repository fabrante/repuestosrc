...
function yiw_register_portoflios(){
    ...
    foreach ( $portofolios as $portfolio ) :   
		...
		add_filter( 'manage_edit-'.sanitize_title( $portfolio->post_title ).'_columns', 'yiw_portfolio_edit_columns');
        ...
        if ( ! empty( $portfolio_tax ) )
            register_taxonomy( sanitize_title( $portfolio_tax ), array( sanitize_title( $portfolio->post_title ) ), array(
        		'hierarchical' => true,
