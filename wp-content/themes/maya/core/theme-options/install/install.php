...
function yiw_import_theme()
{
    ...
    array_walk_recursive( $db, 'yiw_convert_url', $param );
    //yiw_debug($db);
    ...
    foreach( $yiw_wptables as $table )
    {
        ...
        if( $insert )
            yiw_string_( '', sprintf( __( 'Insert new values into %s table', 'yiw' ), $wpdb->$table ), '... Done!</p>' );
        else
    ...
    if ( ! empty( $tables ) )
        foreach ( $tables as $table ) {
            ...
            if( $insert )
                yiw_string_( '', sprintf( __( 'Insert new values into %s table', 'yiw' ), $wpdb->prefix . $table ), '... Done!</p>' );
            else
